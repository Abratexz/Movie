...
const ticketAdmin  = require('../controllers/admin/ticketAdminController');

// Body parsers (safe to enable per-router)
...
router.post('/genres',            genreCtl.create);      
router.post('/genres/new',        genreCtl.create);      
...
router.get('/showtimes/:id/seats.json',   showtimeCtl.seatsJson);     
router.post('/showtimes/:id/seats/reset', showtimeCtl.resetSeats);

/* ---------------- TICKETS (admin list/my tickets) --------------- */
router.get('/tickets',          ticketAdmin.index);
router.get('/tickets/:uid',     ticketAdmin.userTickets);
